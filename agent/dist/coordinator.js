...
export class AgentCoordinator extends EventEmitter {
     ...
     */
    async handleVoiceCommand(sessionId, command) {
        ...
        switch (command.intent) {
            ...
            case 'add_item':
                if (command.items && command.items.length > 0) {
                    ...
                    await this.voiceAgent.announce(sessionId, `Got it! I'll add ${itemsText} to your list.`);
            ...
            case 'manual_control':
                ...
                await this.voiceAgent.announce(sessionId, 'Switching to manual control. Let me know when you want me to take over again.');
                break;
            case 'help':
                await this.voiceAgent.announce(sessionId, 'I can help you shop for groceries, file taxes, renew benefits, or fill government forms. Just tell me what you need!');
                break;
            default:
     ...
     */
    async executeNavigationStep(sessionId) {
        const session = this.sessions.get(sessionId) || this.voiceAgent.getSession(sessionId);
        if (!session) {
        ...
        const { screenshot, url } = await screenshotResponse.json();
        // Plan action using vision agent
        const action = await this.visionAgent.planAction(sessionId, screenshot, url, session.currentTask || 'Complete the task');

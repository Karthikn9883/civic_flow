...
export class VisionAgent {
    ...
    markLastActionResult(sessionId, success) {
        const history = this.sessionHistory.get(sessionId);
        if (history?.length)
            history[history.length - 1].success = success;
    }
     ...
     */
    async planTask(sessionId, screenshotBase64, currentUrl, task) {
...
["step 1 based on what I see", "step 2", ...]`;
    ...
    async planAction(sessionId, screenshotBase64, currentUrl, currentTask) {
        ...
        try {
            ...
            if (!jsonStr)
                throw new Error('No valid JSON in response after retry');
    ...
    buildPrompt(task, url, history, plan) {
        const planSection = plan.length > 0
            ? `\n━━━ TASK PLAN (your roadmap) ━━━\n${plan.map((s, i) => `  ${i + 1}. ${s}`).join('\n')}\n`
            : '';
        const failedCount = history.filter(h => h.success === false).length;
        const historySection = history.length > 0
            ? `\n━━━ ACTIONS TAKEN SO FAR ━━━\n${history.map(h => `  Step ${h.step} [${h.success === false ? '✗ FAILED' : '✓ ok'}]: ${h.action}${h.targetText ? ` "${h.targetText}"` : ''}${h.inputValue ? ` → "${h.inputValue}"` : ''} — ${h.reason}${h.observation ? `\n    Saw: ${h.observation}` : ''}`).join('\n')}\n`
            : '';

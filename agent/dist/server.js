...
async function runVisionLoop(startUrl, task) {
...
app.post('/session/:sessionId/step', async (req, res) => {
    try {
        ...
        const action = await visionAgent.planAction(sessionId, bwData.screenshot, bwData.url || '', 'Complete the current task on screen');
    ...
    ws.on('message', async (raw) => {
        try {
            ...
            if (msg.type === 'realtimeInput' && msg.audioData) {
                ...
                if (!liveSession)
                    return;
                const blob = { data: msg.audioData, mimeType: 'audio/pcm;rate=16000' };

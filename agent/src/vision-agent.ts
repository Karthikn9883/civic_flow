...
export class VisionAgent {
  ...
  async planAction(
    ...
    try {
      ...
      // If model returned plain text, retry once with a stricter prompt
      if (!jsonMatch) {
        console.warn('[VisionAgent] No JSON in response — retrying with strict prompt');
        const retryResult = await this.client.models.generateContent({
          model: this.config.visionModel,
          contents: [{
            role: 'user',
            parts: [
              { inlineData: { data: screenshotBase64, mimeType: 'image/png' } },
              { text: prompt + '\n\nCRITICAL: Your ENTIRE response must be ONLY the JSON object. Start with { and end with }. No other text.' },
            ],
          }],
        });
      ...
      const action: NavigationAction = JSON.parse(jsonMatch[0]);
   ...
   */
  private getPlannerPrompt(currentTask: string, currentUrl: string, history: HistoryEntry[]): string {
    const historySection = history.length > 0
      ? `\nACTIONS ALREADY TAKEN (do NOT repeat these):\n${history.map(h =>
          `  Step ${h.step}: ${h.action}${h.targetText ? ` on "${h.targetText}"` : ''}${h.inputValue ? ` with "${h.inputValue}"` : ''} — ${h.reason}`
        ).join('\n')}\n`
      : '';
...
RESPOND WITH ONLY A JSON OBJECT. Start your response with { and end with }. No text before or after.

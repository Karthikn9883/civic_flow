...
import type { AgentConfig, VoiceCommand, Session } from './types.js';
...
export class VoiceAgent extends EventEmitter {
  ...
  async startSession(sessionId: string): Promise<void> {
    ...
    try {
      ...
      (session as any).liveSession = liveSession;
    } catch (error) {
   ...
   */
  private handleLiveMessage(sessionId: string, message: LiveServerMessage): void {
   ...
   */
  async announce(sessionId: string, message: string): Promise<void> {
    const session = this.sessions.get(sessionId);
    if (!session || !(session as any).liveSession) {
  ...
  async sendAudio(sessionId: string, audioData: string): Promise<void> {
    const session = this.sessions.get(sessionId);
    if (!session || !(session as any).liveSession) return;
